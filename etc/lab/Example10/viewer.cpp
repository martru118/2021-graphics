...
void init() {
	...
	*/
	...
	/*
	 *  Call loadCube to blur the texture that is
	 *  being used as our environment map.
	 */
	blurred = loadCube("VancouverConventionCentre/blurred");
	glGenTextures(1, &blurMap);
	glBindTexture(GL_TEXTURE_CUBE_MAP, blurMap);

	for (i = 0; i < 6; i++) {
		glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, 0, GL_RGBA, blurred->width, blurred->height, 0, GL_RGB, GL_UNSIGNED_BYTE, blurred->data[i]);
	}
...
int main(int argc, char **argv) {
	...
	window = glfwCreateWindow(512, 512, "Viewer", NULL, NULL);
