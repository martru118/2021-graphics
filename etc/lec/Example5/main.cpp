...
void display() {
	...
	model = matrixStack.top(); matrixStack.pop();
	...
	model = glm::rotate(model, 2.3f, glm::vec3(0.0, 1.0, 0.0)); 
	glUniformMatrix4fv(modelLoc, 1, 0, glm::value_ptr(model)); 
	glBindVertexArray(leg->vao); glBindBuffer(GL_ARRAY_BUFFER, leg->ibuffer); 
	glDrawElements(GL_TRIANGLES, leg->indices, GL_UNSIGNED_SHORT, NULL); 
	model = matrixStack.top(); matrixStack.pop();
	...
	model = glm::rotate(model, walk, glm::vec3(0.0, 1.0, 0.0));
...
int main(int argc, char **argv) {
	...
	while (!glfwWindowShouldClose(window)) {
		display(); glfwSwapBuffers(window); 
		glfwPollEvents();
