...
void init() {
	...
	sprintf(fname, "example6c.fs", fragmentName);
...
void display() {
	...
	float lx = cx + 250.0 * cos(angle);
	float lz = cz + 250.0 * sin(angle);
	glUniform3f(LpositionLoc, lx, cy, lz);

	glBindVertexArray(objVAO);
	glDrawElements(GL_TRIANGLES, 3 * triangles, GL_UNSIGNED_INT, NULL);


	if (counter >= 0.0 && counter <= 1.99) {
		colourR = colourR - 0.01;
		colourG = colourG + 0.01;
		//colourB = colourB - 0.01;

		counter = counter + 0.01;
	}
	else if (counter >= 1.99 && counter <= 3.99) {
		//colourR = colourR - 0.01;
		colourG = colourG - 0.01;
		colourB = colourB + 0.01;
	...
	else if (counter >= 3.99 && counter <= 5.99) {
		colourR = colourR + 0.01;
		//colourG = colourG - 0.01;
		colourB = colourB - 0.01;
	...
	glUniform4f(colourLoc, colourR, colourG, colourB, 1.0);
